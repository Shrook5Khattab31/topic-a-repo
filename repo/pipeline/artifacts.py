...
def append_predictions(rows, filename="heldout_predictions.csv"):
    ...
    with open(path, "a", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["id", "y_true", "y_pred", "score", "model_name", "ticket"])
        if write_header:
            w.writeheader()
        for r in rows:
            w.writerow(r)
...
def append_summary(row, filename="summary.csv"):
    ...
    fields = ["ticket", "model_name", "dev_f1_target_1", "heldout_f1_target_1",
...
def append_threshold_sweep(rows, filename="threshold_sweep.csv"):
    ...
    write_header = not path.exists()
    with open(path, "a", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["ticket", "threshold", "precision_target_1", "recall_target_1", "f1_target_1"])
        if write_header:
            w.writeheader()
        for r in rows:
            w.writerow(r)
...
def append_data_quality_audit(rows, filename="data_quality_audit.csv"):
    ...
    write_header = not path.exists()
    with open(path, "a", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["id", "issue_type", "evidence", "disposition", "confidence"])
        if write_header:
            w.writeheader()
        for r in rows:
            w.writerow(r)
