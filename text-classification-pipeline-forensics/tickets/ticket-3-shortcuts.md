...
Prompt/ask: measure keyword-only and length-only signal in isolation, check
redundancy with the text model, and judge whether the resulting shortcut looks
legitimate or artifact-driven.
Output used: `experiments/ticket3_shortcuts.py` (feature-isolation models) and
`experiments/ticket3_validation.py` (redundancy overlap, combined-model McNemar
test, keyword purity table with example inspection).
Verification: the redundancy/combined-model numbers were checked by rerunning the
script and confirming the same F1 and McNemar counts; the legitimacy judgment was
based on directly reading the actual tweet text behind each keyword rather than
inferring it from the purity statistic alone (purity alone can't distinguish
"legitimate disaster word" from "idiom that happens to look disaster-related" —
only reading the tweets can).
